      ...
      homeConfigurations =
        let
          getHomePath = host: user:
            ...
            in
            if builtins.hasAttr "${host}.nix" user-dir then home-dir + /${user}/${host}.nix
            else if builtins.hasAttr "generic.nix" user-dir then home-dir + /${user}/generic.nix
            else null;
          getHostUsers = host: (builtins.attrNames (lib.filterAttrs
            (n: v: v.group == "users")
            outputs.nixosConfigurations.${host}.config.users.users));
          host_users = builtins.filter (v: v.config_path != null)
